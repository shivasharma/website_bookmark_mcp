...
server.tool("save_bookmark", "Save a website URL as a bookmark. Provide the URL and optional metadata like title, description, tags, and notes.", {
...
server.tool("list_bookmarks", "List all saved bookmarks. Optionally filter by search query, tag, or show only favorites.", {
    ...
    try {
        ...
        const lines = bookmarks.map((bookmark, index) => `${index + 1}. [ID:${bookmark.id}] ${bookmark.is_favorite ? "STAR " : ""}${bookmark.title || bookmark.url}\n` +
            `   URL: ${bookmark.url}\n` +
            `   Tags: ${bookmark.tags.length ? bookmark.tags.join(", ") : "none"}\n` +
...
server.tool("update_bookmark", "Update an existing bookmark by its ID. You can change the title, description, tags, notes, or mark it as favorite.", {
